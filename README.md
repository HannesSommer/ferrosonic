...
- **First play costs a second download.** Playback never waits for the cache: mpv starts streaming immediately while the copy is fetched in the background. That means a track's first play transfers it twice. Every play after that transfers nothing.
